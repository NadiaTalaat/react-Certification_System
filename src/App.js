...
export default function CertificatePortal() {
  ...
  const disconnectSupabase = () => {
    if (window.confirm('Switch to Browser Storage?')) {
    ...
    try {
      if (supabaseConfigured && supabaseUrl && supabaseKey) {
        const response = await fetch(
          `${supabaseUrl}/rest/v1/certificates?select=*`,
          {
            headers: {
              apikey: supabaseKey,
  ...
  const handleLogin = () => {
    ...
    setUser({ email: loginEmail.toLowerCase() });
    setIsAdmin(loginEmail.toLowerCase() === ADMIN_EMAIL);
  };
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setNewCert({ ...newCert, certificateFile: reader.result });
      };
  ...
  const handleAddCertificate = async () => {
    if (!newCert.studentEmail || !newCert.certificateType) {
      alert('Please fill in Email and Certificate Type');
      return;
    }

    if (!newCert.certificateLink && !newCert.certificateFile) {
      alert('Please either upload a file OR provide a link');
      return;
    }
    ...
    const certificate = {
      ...
      studentEmail: newCert.studentEmail.toLowerCase(),
      studentName: newCert.studentName || '',
      certificateType: newCert.certificateType,
      certificateLink: newCert.certificateFile || newCert.certificateLink,
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (values.length >= 3) {
          const cert = {
            ...
            studentEmail: values[0].toLowerCase(),
            studentName: values[1] || '',
            certificateType: values[2] || '',
            certificateLink: values[3] || '',
      ...
      alert(`Successfully uploaded ${newCerts.length} certificates!`);
      setBulkData('');
  ...
  const handleDeleteCertificate = async (id) => {
    ...
    try {
      if (supabaseConfigured && supabaseUrl && supabaseKey) {
        await fetch(`${supabaseUrl}/rest/v1/certificates?id=eq.${id}`, {
          method: 'DELETE',
          headers: {
            apikey: supabaseKey,
            Authorization: `Bearer ${supabaseKey}`,
  ...
  const filteredCertificates = certificates.filter((cert) => {
    const matchesUser = isAdmin || cert.studentEmail === user?.email;
    const matchesSearch =
      !searchTerm ||
      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Access your digital certificates</p>
          </div>
          ...
          <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-100">
            <p className="text-sm text-gray-700">
              <strong>Demo:</strong> Use any email to view your certificates
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  {isAdmin ? 'Admin Dashboard' : 'My Certificates'}
                </h1>
                {isAdmin && (
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
          ...
          {isAdmin && (
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center space-x-2 px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition shadow-md"
              >
              ...
              <button
                ...
                className="flex items-center space-x-2 px-4 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition shadow-md"
              >
              ...
              <div className="space-y-4">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <code className="text-xs bg-white p-2 block rounded">
                    email,name,type,link,date
                    <br />
                    student1@gmail.com,John Doe,Web Dev,https://...,2026-01-08
                    <br />
                    student2@gmail.com,Jane Smith,Data
                    Science,https://...,2026-01-08
                  </code>
                  <p className="text-xs text-gray-600 mt-2">
                    💡 <strong>Tip:</strong> Use ONE Google Drive folder link
                    for all certificates, or leave links blank if uploading
                    files individually
                  </p>
                </div>
        ...
        {/* Add Single Certificate Modal - ADMIN ONLY */}
        {showAddForm && isAdmin && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 max-h-screen overflow-y-auto">
              ...
              <div className="space-y-4">
                <input
                  ...
                  onChange={(e) =>
                    setNewCert({ ...newCert, studentEmail: e.target.value })
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewCert({ ...newCert, certificateType: e.target.value })
                  }
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="ml-11 space-y-1 text-sm text-gray-600">
                        ...
                        {isAdmin && (
                          <p>
                            <strong>Email:</strong> {cert.studentEmail}
                          </p>
                        )}
                        <p>
